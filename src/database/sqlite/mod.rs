...
impl SqliteDb {
    ...
    /// passed through the `bitcoin_txs` parameter otherwise the transaction will fail.
...
";
    ...
    fn v4_to_v5_migration() {
        ...
        // Create a hundred different transactions, from which originates two hundreds
        // pseudo-random coins.
        ...
        let spend_tx = bitcoin::Transaction {
            version: bitcoin::transaction::Version::TWO,
            lock_time: bitcoin::absolute::LockTime::from_height(1_234).unwrap(),
            input: Vec::new(),
            output: Vec::new(),
        };
        let coins: Vec<Coin> = bitcoin_txs
            ...
            .map(|(i, tx)| Coin {
                ...
                block_info: if i & 2 == 0 {
                    Some(BlockInfo {
                        height: i as i32 * 1_000,
                        time: 1722408619 + i as u32 * 42_000,
                    })
                } else {
                    None
                },
                ...
                spend_block: if i % 40 == 0 {
                    Some(BlockInfo {
                        height: i as i32 * 2_000,
                        time: 1722488619 + i as u32 * 84_999,
                    })
                } else {
                    None
                },
...

        {
            ...
            let confirmed_coins: Vec<_> = coins
                .iter()
                .filter_map(|coin| {
                    if let Some(blk) = coin.block_info {
                        Some((coin.outpoint, blk.height, blk.time))
                    } else {
                        None
                    }
                })
            ...
            let confirmed_spent_coins: Vec<_> = coins
                .iter()
                .filter_map(|coin| {
                    if let Some(blk) = coin.spend_block {
                        Some((
                            coin.outpoint,
            ...
            let coins_txids: Vec<_> = conn.db_coins(&[]).into_iter().map(|c| c.outpoint).collect();
        }
        ...
        // Migration with all txs witll succeed.
        bitcoin_txs.push(spend_tx);
        maybe_apply_migration(&db_path, &bitcoin_txs).unwrap();

        // Make sure all the transactions are indded in DB.
        {
            let db = SqliteDb::new(db_path.clone(), None, &secp).unwrap();
            let mut conn = db.connection().unwrap();

            let txids: Vec<_> = bitcoin_txs.iter().map(|tx| tx.txid()).collect();
            let mut bitcoin_txs_in_db: Vec<_> = conn
                .list_wallet_transactions(&txids)
                .into_iter()
                .map(|tx| tx.transaction)
                .collect();
            bitcoin_txs_in_db.sort();
            bitcoin_txs.sort();
