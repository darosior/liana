...
fn migrate_v2_to_v3(conn: &mut rusqlite::Connection) -> Result<(), SqliteDbError> {
    db_exec(conn, |tx| {
        tx.execute(
            "CREATE TABLE labels (id INTEGER PRIMARY KEY NOT NULL, wallet_id INTEGER NOT NULL, labelled_kind INTEGER NOT NULL CHECK (labelled_kind IN (0,1,2)), labelled TEXT UNIQUE NOT NULL, value TEXT NOT NULL)",
            rusqlite::params![],
        )?;
